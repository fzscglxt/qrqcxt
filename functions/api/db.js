/**
 * QRQC快速反应管理系统 - 云端数据同步 API (Cloudflare Pages Functions)
 *
 * 接口规范：
 ...
 * 绑定一个 KV 命名空间，变量名设为 FZ_QRQC_DB。
 */

const KV_KEY = 'qrqc_data';
const CORS_HEADERS = {
...
export async function onRequestGet(context) {
  const kv = context.env.FZ_QRQC_DB;
  if (!kv) {
    return jsonResponse({ error: 'kv_not_bound', message: 'KV 命名空间 FZ_QRQC_DB 未绑定，请在 Pages 项目设置中配置' }, 500);
  }
